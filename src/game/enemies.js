...
const Enemies = (() => {
    ...
    function getGroundHeight(x, z) {
        ...
        const model = sceneRef.getObjectByName("environment_model");
        if (!model) return 0;
        
        const origin = new THREE.Vector3(x, 30, z);
    ...
    function spawnInsect(x, y, z, labelText, typeId, hpVal = 60) {
        ...
        group.position.set(finalX, y + 0.8, finalZ);
    ...
    function spawnDrop(pos) {
        ...
        keyGroup.position.y = Math.max(keyGroup.position.y, 0.8);
        
        const torus = new THREE.Mesh(keyTorusGeo, goldMaterial);
        ...
        if (Math.random() < 0.5) {
            ...
            extraPos.y = Math.max(extraPos.y, 0.8);
            ...
            if (rand < 0.5) {
                ...
                const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.1, 0.56), crossMaterial);
                const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.4, 0.56), crossMaterial);
                hpGroup.add(crossH);
                hpGroup.add(crossV);

                sceneRef.add(hpGroup);

                const extraLight = new THREE.PointLight(0x0ea5e9, 1.8, 4);
