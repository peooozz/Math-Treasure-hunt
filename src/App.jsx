...
function App() {
    ...
    useEffect(() => {
        ...
        const drawMinimap = () => {
            ...
            const { grid, playerPos, enemies, vaults, portalActive, portalCell, yaw } = data;
            ...
            if (portalStatusEl) {
                if (portalActive) {
                    ...
                    portalStatusEl.style.color = "#94a3b8"; // slate color
                }
            }
            ...
            // Draw grid walls and pillars
            for (let r = 0; r < rows; r++) {
                for (let c = 0; c < cols; c++) {
                    const cell = grid[r][c];
                    if (cell === 1 || cell === 2) {
                        ctx.fillStyle = cell === 1 ? 'rgba(255, 0, 122, 0.4)' : 'rgba(168, 85, 247, 0.5)';
                        ctx.fillRect(c * cellW, r * cellH, cellW, cellH);
                        ctx.strokeStyle = cell === 1 ? 'rgba(255, 0, 122, 0.75)' : 'rgba(168, 85, 247, 0.85)';
            ...
            const worldToGrid = (worldX, worldZ) => {
                return {
                    x: ((worldX + halfW) / 4) * cellW,
                    y: ((worldZ + halfD) / 4) * cellH
                };
            };
            ...
            vaults.forEach(v => {
                if (v.unlocked) return;
                const pos = worldToGrid(v.position.x, v.position.z);
                ctx.beginPath();
                ...
                ctx.strokeStyle = v.opened ? 'rgba(57, 255, 20, 0.6)' : 'rgba(189, 0, 255, 0.6)';
                ctx.stroke();
            });
            ...
            enemies.forEach(e => {
                if (e.dead) return;
                const pos = worldToGrid(e.mesh.position.x, e.mesh.position.z);
                ctx.beginPath();
            ...
            if (portalActive && portalCell) {
                ...
                const pos = worldToGrid(portalWorldX, portalWorldZ);
                ctx.beginPath();
                ctx.arc(pos.x, pos.y, 5, 0, Math.PI * 2);
            ...
            if (playerPos) {
                const pos = worldToGrid(playerPos.x, playerPos.z);
                ctx.save();
            ...
            {gameStarted && (
                <>
                    ...
                    <div id="hud" className={gravityInverted ? 'gravity-inverted' : ''}>
                        ...
                        <div className="hud-panel left-panel nav-panel font-rajdhani" style={{ marginTop: '10px' }}>
                            <div className="stat-header font-orbitron" style={{ fontSize: '0.8rem', color: 'var(--color-primary)', borderBottom: '1px solid rgba(0,240,255,0.2)', paddingBottom: '3px', fontWeight: 'bold' }}>
                                🛰️ NAVIGATION SYSTEMS
                            </div>
                            <div style={{ fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span>🧭 Heading:</span>
                                    <span id="nav-heading" className="text-gold font-orbitron">0° N</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span>🎯 Nearest Vault:</span>
                                    <span id="nav-nearest-vault" className="font-orbitron" style={{ color: 'var(--color-primary)' }}>N/A</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span>🌀 Portal Status:</span>
                                    <span id="nav-portal-status" className="font-orbitron">OFFLINE</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="hud-panel right-panel">
                            <div className="minimap-container">
                                <canvas ref={minimapRef} width="160" height="160" id="minimap-canvas"></canvas>
                            </div>
                            <div className={gravityInverted ? 'gravity-flipped font-orbitron' : 'gravity-normal font-orbitron'}>
                                {gravityInverted ? '▲ GRAVITY: INVERTED (-1G)' : '▼ GRAVITY: NORMAL (1G)'}
                            </div>
                            <div className="orb-container">
                                <div className={orbs > 0 ? 'orb-ready font-orbitron' : 'font-orbitron'} id="orb-icon">
                                    🌀 ORBS
                                    <div id="orb-count-badge">{orbs}</div>
                                </div>
                                {gravityInverted && (
                                    <div id="orb-timer-container" style={{ display: 'flex' }}>
                                        <svg width="40" height="40" viewBox="0 0 40 40">
                                            <circle className="timer-bg" cx="20" cy="20" r="16"></circle>
                                            <circle 
                                                className="timer-fg" 
                                                cx="20" 
                                                cy="20" 
                                                r="16"
                                                style={{ strokeDashoffset: svgOffset, strokeDasharray: svgCircumference }}
                                            ></circle>
                                        </svg>
                                        <span id="orb-timer-text" className="font-orbitron">{Math.ceil(gravityDuration)}s</span>
                                    </div>
                                )}
                            </div>
                        </div>
